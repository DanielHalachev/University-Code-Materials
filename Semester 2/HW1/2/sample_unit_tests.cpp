...
void runTests() {
    ...
    suite.removeByErrorType(ErrorType::None);
    assert(suite.filterPassing().size() == 0);
    std::cout << suite.filterPassing().size() << std::endl;
    std::cout << suite.filterFailing().size() << std::endl;
}
